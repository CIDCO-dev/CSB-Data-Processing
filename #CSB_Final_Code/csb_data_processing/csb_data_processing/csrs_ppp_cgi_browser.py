...
# ----------------------------------------------------------------------------------------------------------------------
#ie. PPP_working.py --user_name k.a@unb.ca --lang en --ref NAD83 --epoch CURR --mode Kinematic --rnx 'C:\PPP\13_46_34-2017_09_22-gps.17o' --path 'C:\PPP'
    ...
    if req.text:
        ...
        if not error:
            ...
            for wget_num in range(wget_max):
                ...
                # wget
                log = Popen(command, stdout=PIPE, shell=True)
                output = log.communicate()
                output = output[0].decode('utf-8')
                if output and debug:
                    print('log[{0:d}]: {1:s}'.format(wget_num, log))
