from hycom_lookup_table import Hycom_Lookup_Update
...
class Hycom_SVP_Query:
	...
	def __init__(self, srt_file_name, working_dir):
		self.srt_file_name = srt_file_name
		self.working_dir = working_dir
		self.hycom_url = self.gbl_hycom
		self.lookup_table_instance = Hycom_Lookup_Update() #updates lookup table
		self.lookup_table_instance = self.lookup_table_instance.return_lookup_table() #loads lookup table into memory
		...
		print 'Hycom_SVP_Query: initialized'

	#open srt file, load contents to memory
	def load_srt_contents(self):
		print 'Hycom_SVP_Query: entering load_srt_contents() script'

		file_exists = os.path.exists(self.working_dir + self.srt_file_name)

		if file_exists:
			srt_file_path = self.working_dir + self.srt_file_name
			srt_file = open(srt_file_path,"r")
			self.srt_contents = srt_file.readlines()
			srt_file.close() # close the file
			print 'read srt file successfully'
			print 'Hycom_SVP_Query: exiting load_srt_contents() script'

		else:
			io_error = "IOError: check that file {} or destination {} exists".format(self.srt_file_name, self.working_dir) 
			print io_error
			return io_error
	...
	def identify_srt_header(self):
		print 'Hycom_SVP_Query: entering identify_srt_header() script'
		...
		for command in self.index.keys():
			...
			if self.srt_contents:
				...
				value_error = "ValueError: there was an error reading in {}. Please check contents and retry".format(self.srt_file_name)
		
		self.header_variables = self.srt_contents[self.index['header']]
		self.header_variables = self.header_variables.split()
		print 'Hycom_SVP_Query: exiting identify_srt_header() script'
	...
	def rinex_iterator(self):
		print 'Hycom_SVP_Query: entering rinex_iterator() script'
		...
		if self.connect_to_hycom(): 
			for counter, line in enumerate(file_data, self.index['gnss']):
				if line == '#FOR TIDAL VERTICAL REFERENCING:':
					#skip the poorly qualified gnss readings
					break
					...
					time_index = self.find_time_index(epoch_hours)
					print 'successfully created time index'
					...
					print 'successfully created latitude and longitude index'
	
					#create hycom depth index for opendap server query
					depth_index = '0:39'
					print 'successfully created depth index'
	
					#final query
					hycom_query = [time_index, depth_index, lat_index, lon_index] #raw values
					print 'raw query without rounding: {}'.format(hycom_query)
					hycom_query = [int(time_index), depth_index, int(lat_index), int(lon_index)] #rounded floor
					print 'final query with rounding: {}'.format(hycom_query)
					...
					if hycom_query in self.hycom_query_cache:
						...
						print 'unique query appended to cache'
			...
			if self.rinex_iterator_retry_count == self.rinex_iterator_retry_attempts:
				...
				print 'attempting to retry connection to Hycom after {} tries'.format(self.rinex_iterator_retry_count)
				self.rinex_iterator()
		...
		if not self.hycom_query_flag: #fix, will execute search based on last hycom query in cache; can query each value in cache and average svp, fix, only append queries successful
			print 'no results found, perform radial search for data...'
			self.search_hycom()
			print 'radial search function complete'
		
		#take successful queries and get temperature values
		self.query_hycom_temperature()

		#save queried data as caris svp
		print 'Hycom queries completed, now complete svp'
	...
	def retrieve_line_values(self,line):
		...
		print 'successfully retrieved line values'
	...
	#helper: searches the lookup table for the corresponding hycom time index
	def find_time_index(self, epoch_hours):
		#find min and max epoch hours, set as variable
		min_epoch = self.hycom_date_zero
		if self.hycom_latest_date == 157812:
			max_epoch = max(self.lookup_table_instance)
			self.hycom_latest_date = max_epoch
		else:
			max_epoch = self.hycom_latest_date

		#check if query is within min and max range
		if epoch_hours > min_epoch and epoch_hours < max_epoch:
			#find floor for query in range, consider in the future to implement as binary tree
			lookup_table_keys = sorted(self.lookup_table_instance.keys())
			index_key = max(x for x in lookup_table_keys if x < epoch_hours)
			index_value = self.lookup_table_instance[index_key]
			return index_value
		else:
			print "ERROR: epoch given not within range"
			return False
	...
	def connect_to_hycom(self):
		print 'attempting to connect to Hycom url {}'.format(self.gbl_hycom)
		try:
			self.hycom_db_handle = open_url(self.gbl_hycom)
			print 'successfully conected to Hycom url {}'.format(self.gbl_hycom)
			return True
		except:
	...
	def query_hycom_salinity(self):
		...
		if not salinity[0][0][0] == -30000:
			...
			print 'salinity query successfully returned value'
		else:
			self.hycom_query_flag.append(False)
			print 'salinity query returned null value'
		...
		for flag,query in zip(flag,query):
			if flag:
				...
				if not temperature[0][0][0] == -30000:
					...
					print 'temperature query successfully returned value'
		...
		for flag,query in zip(flag,query):
			if not flag:
				...
				while not self.hycom_query_flag[-1]:
					print 'entering radial search function..'
					if index == 0:
	...
	def save_output_svp(self):
		...
		for count, (sal,temp) in enumerate(zip(salinity,temperature)):
			...
			svp_file_path = self.working_dir + new_file_name
			...
			fn.close()
			print 'svp created successfully as {}'.format(new_file_name)
	...
	def format_svp_header(self, count):
		...
		print 'created svp header successfully'
		return comment
